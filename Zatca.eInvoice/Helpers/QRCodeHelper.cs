...
namespace Zatca.eInvoice.Helpers
{
    internal static class QRCodeHelper
    {
        ...
        {
            var details = new SortedDictionary<int, string[]>();

            try
            {
                byte[] data = Convert.FromBase64String(qrCodeBase64);
                ...
                while (index < data.Length)
                {
                    ...
                    details.Add((int)tag, [Encoding.UTF8.GetString(value)]);
                }
                ...
                {
                    Console.WriteLine($"Tag {detail.Key}: {string.Join(", ", detail.Value)}");
                }
            }
            catch (Exception ex)
            {
